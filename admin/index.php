...
<head>
    ...
    <style>
        ...
        /* Deposit - Verde */
        .status-indicator.deposit {
            color: #00d4aa;
        }
        
        /* Withdrawal - Laranja */
        .status-indicator.withdrawal {
            color: #ff9500;
        }
    ...
    <main class="main-content">
        ...
        <div class="row">
            <div class="col-md-6 mb-4">
                <div class="table-container">
                    ...
                    <table class="custom-table">
                        <colgroup>
                            <col style="width: 30%;"> <!-- Usuário -->
                            <col style="width: 40%;"> <!-- Email -->
                            <col style="width: 30%;"> <!-- Data -->
                        </colgroup>
                        <thead>
                            <tr>
                                <th>Usuário</th>
            ...
            <div class="col-md-6 mb-4">
                <div class="table-container">
                    <div class="table-header">
                        <h4 class="table-title">Últimas 5 Transações</h4>
                    </div>
                    <table class="custom-table">
                        <colgroup>
                            <col style="width: 25%;"> <!-- Usuário -->
                            <col style="width: 20%;"> <!-- Tipo -->
                            <col style="width: 30%;"> <!-- Valor -->
                        ...
                        <tbody>
                            <?php
                            // Query para buscar os últimos 5 depósitos e saques combinados
                            $transacoes_query = "(
                                SELECT 
                                    d.id,
                                    d.amount as valor,
                                    d.status,
                                    d.created_at as data_transacao,
                                    u.name as usuario_nome,
                                    'depósito' as tipo_transacao
                                FROM deposits d
                            ...
                            (
                                SELECT 
                                    ...
                                    s.valor,
                                    s.status,
                                    s.data_solicitacao as data_transacao,
                                    u.name as usuario_nome,
                            ...
                            LIMIT 5";
                            $transacoes_result = $conn->query($transacoes_query);
                            
                            if ($transacoes_result && $transacoes_result->num_rows > 0) {
                                while($transacao = $transacoes_result->fetch_assoc()) {
                                    echo "<tr>";
                                    echo "<td>" . htmlspecialchars($transacao['usuario_nome'] ?? 'Usuário não encontrado') . "</td>";
                                    ...
                                    if ($status == 'aprovado' || $status == 'pago' || $status == 'concluido') {
                                        echo "<td><span class='status-indicator success' title='" . ucfirst($transacao['status']) . "'><i class='bi bi-check-circle-fill'></i></span></td>";
                                    } elseif ($status == 'pendente') {
                                        echo "<td><span class='status-indicator pending' title='Pendente'><i class='bi bi-clock-fill'></i></span></td>";
                                    } else {
                                        echo "<td><span class='status-indicator error' title='" . ucfirst($transacao['status']) . "'><i class='bi bi-x-circle-fill'></i></span></td>";
                                    }
                                ...
                                echo "<tr><td colspan='4' class='text-center' style='padding: 2rem; color: var(--dark-text-secondary);'><i class='bi bi-inbox' style='font-size: 2rem; margin-bottom: 1rem; display: block;'></i>Nenhuma transação encontrada</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>
